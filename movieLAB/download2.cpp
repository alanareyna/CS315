...
//    }
    ...
    int getFrequentRenterPoints(int numDaysRented) {
        int frequentRenterPoints = 1;
        // add bonus for a two day new relase rental
        if(getPriceCode() == Movie::NEW_RELEASE && numDaysRented > 1 )
...
//    }
    ...
    string statement() {
        ...
        for( auto thisRental:  _rentals ) {
            ...
            double thisAmount = thisRental->getCharges();
	        frequentRenterPoints += thisRental->getFrequentRenterPoints( thisRental );
